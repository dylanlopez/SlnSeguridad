...
{
    public class TipoInstitucionController : ApiController
    {
        private TipoInstitucionModel _model;
        private DataContractJsonSerializer _jsonSerializer;
        private BRestOperation _restOperation;

        [HttpPost]
        [Authorize]
        public HttpResponseMessage Actualizar(TipoInstitucionModel model)
        {
            _model = model;
            try
            {
                var path = ConfigurationManager.AppSettings["WCFPath"].ToString();
                path = path + "Services/SPersonasService.svc/ActualizarTipoInstitucion/";
        ...
        [HttpPost]
        [Authorize]
        public HttpResponseMessage Insertar(TipoInstitucionModel model)
        {
            try
            {
                _model = model;
                var path = ConfigurationManager.AppSettings["WCFPath"].ToString();
                path = path + "Services/SPersonasService.svc/InsertarTipoInstitucion/";
                var dataToSend = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(_model));
                using (_restOperation = new BRestOperation())
                {
                    var stream = _restOperation.Post(path, dataToSend);
        ...
        [HttpPost]
        [Authorize]
        public List<TipoInstitucionModel> Listar(TipoInstitucionModel model)
        {
            List<TipoInstitucionModel> response;
            _model = model;
            try
            {
                var path = ConfigurationManager.AppSettings["WCFPath"].ToString();
                path = path + "Services/SPersonasService.svc/ListarTipoInstituciones/";
                var dataToSend = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(_model));
                using (_restOperation = new BRestOperation())
                {
                    var stream = _restOperation.Post(path, dataToSend);
                    _jsonSerializer = new DataContractJsonSerializer(typeof(List<TipoInstitucionModel>));
                    response = (List<TipoInstitucionModel>)_jsonSerializer.ReadObject(stream);
                    return response;
                }
            }
            catch (Exception ex)
